...
6. **Accessibility tuzatishlari** (TZ 16-bo'lim): global `:focus-visible` uslubi qo'shildi (ilgari faqat aloqa formasida bor edi, nav/link/button'larda YO'Q edi) — `static/site.css`. "Skip to content" havolasi (`templates/base.html`, `id="main-content"`) klaviatura foydalanuvchilari uchun. Bosh navigatsiyaning `aria-label`i noto'g'ri edi (xizmatlar matniga bog'langan) — endi "Asosiy navigatsiya"ga tuzatildi. Yangi matnlar `app/content/common.py::COMMON` (3 tilda).
7. **Blog post slug-redirect** (TZ 27-bo'lim): Services/Portfolio'dan farqli, blog posti slug'i sarlavhadan avtomatik hosil bo'ladi (`pages._slugify`) — sarlavha tahrirlansa slug o'zgarardi va ESKI havola 404 bo'lib qolardi, hech qanday redirect yo'q edi (mavjud `SlugRedirect` infratuzilmasi ishlatilmagan edi). Endi `update_post` sarlavha/til o'zgarganda avtomatik 301 yozadi, `blog_detail` redirect jadvalini tekshiradi (Services/Portfolio bilan bir xil pattern).
8. **To'liq funksional test o'tkazildi** — lokal serverni ishga tushirib (haqiqiy `promtchi.db`ning ALOHIDA nusxasida — asl fayl tegilmagan, Telegram bot tokeni nusxada bo'shatildi — real xabarnoma yuborilmasligi uchun), httpx orqali **100 ta avtomatik tekshiruv**: barcha 24 sahifa (8 route × 3 til), sitemap/robots validligi, xavfsizlik header'lari, 404, hreflang o'zaro bog'lanishi va JSON-LD validligi, aloqa formasi (to'g'ri/xato holatlar, honeypot), admin login (to'g'ri/xato/token yo'q), Services/Portfolio/Post to'liq CRUD (admin.html'ning HAQIQIY ma'lumot oqimini simulyatsiya qilib — aynan shu usul yuqoridagi slug xatosini topgan edi), FAQ content-editor, fayl yuklash (ruxsat etilgan/etilmagan formatlar), CRM leads/meta, fikr (review) kod bilan yozish + qayta ishlatishni rad etish, tezkor so'rovlarda rate-limit (429) ishlashi — **hammasi muvaffaqiyatli**. Topilgan 2 ta "xatolik" test skriptining o'zidagi xato edi (case-sensitive regex; `/docs` ENV=development'da ataylab yoqilgan) — tekshirilib, real muammo emasligi tasdiqlandi.
   - **DIQQAT**: Chrome kengaytmasi bu sessiyada ulanmagan edi — shu sabab faqat HTTP/funksional daraja test qilindi (barcha marshrut, forma, CRUD, xavfsizlik). Vizual/render (rang, joylashuv, responsive breakpoint, animatsiya, haqiqiy klik/hover) tekshiruvi QILINMADI — kengaytma ulangach alohida vizual audit tavsiya etiladi.
   - **Kuzatuv** (o'zgartirilmadi): `.env`da `ENABLE_DOCS=true` `ENV=production` bilan birga turibdi — production'da `/openapi.json`/`/docs` ochiq qoladi (kod xavfsiz default — `ENV=production` + `ENABLE_DOCS` berilmasa avtomatik yopiladi — buni tasdiqladim, lekin `.env` buni qo'lda qayta yoqqan). Xohlasangiz shu qatorni o'chirib qo'yishingiz mumkin.

**2026-09-16 kuni (davomi) bajarildi — audit'da "hali ochiq" deb belgilangan barcha texnik masalalar:**
9. **`/uz/jamoa/` route qo'shildi** (TZ 3.2-bo'lim). Real jamoa profillari alohida sahifa uchun hali tayyor emasligi sababli (TZ 12/29 — rozilik kutilmoqda), bo'sh/thin sahifa emas — `/{lang}/biz-haqimizda/` dagi mavjud "Bizning ekspertiza" blokiga 301 redirect (`app/pages.py::team_redirect`). Profillar tasdiqlangach shu yerga to'liq sahifa qo'shish mumkin.
10. **Homepage "Loyihalar" grid — Portfolio DB bilan mos qilindi va crawl qilinadigan bo'ldi** (TZ 3.2/4.5/27-bo'lim). Muammo: `Content.data.cases` (bosh sahifa, admin "Loyihalar" tab) va yangi `PortfolioCase` DB (TZ 4/19) ikkita mustaqil manba edi — uchinchi case ikkalasida boshqa-boshqa loyiha edi ("BotSmith Platform"/xato yozilgan "Tizimliy" — Telegram bot generatori haqida, aslida real Portfolio DB'dagi "Tizimly" B2B SaaS platforma bilan hech aloqasi yo'q edi). Tuzatildi:
    - `static/index.html`/`.ru.html`/`.en.html` DEFAULTS.cases uchinchi case'i haqiqiy "Tizimly" (savdo/ombor/moliya B2B SaaS, 150+ kompaniya) matniga almashtirildi, barcha 3 case'ga `slug` (chindan-group/notiq-ai/tizimly) qo'shildi.
    - `app/db.py::run_data_fixups()` — ishlab turgan bazalardagi eski `Content.data.cases` yozuvini bir martalik avtomatik tuzatadi (idempotent, sinov qilindi).
    - Loyiha kartalari endi JS modal o'rniga to'g'ridan-to'g'ri `/{lang}/portfolio/{slug}/` sahifasiga link beradi (`<button onclick>` → `<a href>`) — TZ 27: "faqat anchor orqali... yetarli emas", bu ilgari blog uchun qilingan tuzatish bilan bir xil mantiq. Endi ishlatilmay qolgan case-modal (`openCase`/`closeCase`/`#caseModal`) barcha 3 tilda olib tashlandi.
    - `app/schemas.py::CaseIn`ga `slug` maydoni qo'shildi (admin "Loyihalar" tabidan saqlashda endi yo'qolib qolmaydi).
11. **FAQ to'liq DB CRUD'ga o'tkazildi** (TZ 19-bo'lim — ilgari faqat bosh sahifadagi 3 ta qisqa savol admin-tahrirlanardi, `/faq/`dagi 24 ta to'liq savol-javob qattiq yozilgan edi). Yangi `app/db.py::FaqItem` jadvali + `app/faq_store.py` (Service/PortfolioCase bilan bir xil naqsh: xotira keshi, bir martalik seed) + `GET/POST/PUT/DELETE /api/admin/faq[/{id}]` (`app/main.py`) + `static/admin.html`da yangi "FAQ sahifasi" bo'limi (mavjud "FAQ" tab endi "FAQ (bosh sahifa, qisqa)" deb aniqlashtirildi — TZ 4.8 ikkalasini ham ruxsat beradi, faqat maqsadi farqli). `app/pages.py::faq_page` endi `faq_store`dan o'qiydi; `app/content/faq.py` faqat bir martalik seed manbai (Service/Portfolio namunasi bilan bir xil). To'liq test qilindi: CRUD, publish/unpublish, duplicate-key guard, FAQPage schema sinxronligi — hammasi httpx orqali tasdiqlandi.
12. **TZ 2-bo'lim majburiy huquqiy/va'da tuzatishlari**: "Kod huquqi to'liq mijozga tegishli" (qat'iy da'vo) → shartnomaga bog'langan neytral matn; bosh sahifadagi qisqa FAQ'dagi blanket "3 oy bepul o'zgartirishlar" → to'liq FAQ'dagi paket-darajali (14/30/90 kun) matn bilan mos qilindi. Tuzatildi: `app/content/faq.py` (3 tilda), `app/schemas.py::DEFAULT_CONTENT`, `static/index.html` DEFAULTS.faq, va `app/db.py::run_data_fixups()` orqali ishlab turgan baza uchun ham avtomatik.
    - Barcha o'zgarishlar test qilindi: `pytest tests/` (23/23 o'tdi), yangi FaqItem CRUD/migratsiya alohida sinov skriptlari bilan, JS fayllari `node --check` bilan sintaksis tekshiruvidan o'tdi.
    - **Kuzatuv**: Chrome kengaytmasi bu sessiyada ham ulanmagan — admin panelning yangi "FAQ sahifasi" bo'limi vizual/klik darajada tekshirilmadi (faqat HTML/JS sintaksisi va backend API to'liq test qilindi, xuddi shu CSS klasslari Xizmatlar/Portfolio bo'limlaridan qayta ishlatilgani uchun render muammosi kutilmaydi).

**2026-09-16 kuni (3-bosqich) bajarildi — biznes bilan kelishilgan qaror (4-band): "RU/EN homepage'da admin panel o'zgarishlari avtomatik bo'lishi kerak":**
13. **Content — to'liq trilingual CMS'ga o'tkazildi** (TZ 1-bo'lim, oldingi bosqichda "yirik arxitektura o'zgarishi" deb qoldirilgan band). Muammo: `packages`/`team`/`testimonials`/bosh sahifadagi qisqa `faq` FAQAT o'zbekcha (`Content.data`) saqlanardi — RU/EN sahifalari mustaqil, qattiq-yozilgan tarjimaga tayanardi; admin panelda biror narsa o'zgartirilsa, faqat UZ sahifasida ko'rinardi. Tuzatildi:
    - `app/schemas.py::ContentDoc` — `packages`/`team`/`testimonials`/`faq` endi `{uz:[...], ru:[...], en:[...]}` shaklida (`PackagesByLang`/`TeamByLang`/`TestimonialsByLang`/`FaqTeaserByLang`); `contacts`/`socials` o'zgarishsiz qoldi (tilga bog'liq emas — telefon/telegram raqami tarjima qilinmaydi). `CaseIn`/`cases` maydoni BUTUNLAY olib tashlandi.
    - `DEFAULT_CONTENT` (schemas.py) — RU/EN uchun ilgari `static/index.ru.html`/`.en.html`da qattiq yozilgan PROFESSIONAL tarjimalar (TZ 22: avtomatik tarjima emas) shu yerga ko'chirildi.
    - `app/db.py::run_data_fixups()` — ishlab turgan bazadagi eski flat (faqat-uz) shaklni yangi til-bo'yicha shaklga bir martalik, idempotent ko'chiradi (sinov qilindi: eski shakl → yangi shakl, qayta ishga tushirilsa o'zgarish yo'q); `cases` maydonini butunlay o'chiradi.
    - `app/main.py::_LangContentCache` — `Content` hujjatini xotirada til bo'yicha saqlaydi; `GET /api/content?lang=uz|ru|en` endi so'ralgan tilni tekislab qaytaradi, `cases`ni esa har so'rovda to'g'ridan-to'g'ri Portfolio bazasidan (`services_store`, o'z keshi bilan — DB'ga tegmaydi) qo'shib beradi — endi bitta yagona manba, ikkinchi parallel "Loyihalar" ro'yxati YO'Q.
    - Yangi `GET /api/admin/content` — admin panel uchun XOM (barcha 3 til) hujjatni qaytaradi; `PUT`/`POST /reset` o'zgarishsiz (endi yangi shaklni qabul qiladi).
    - `static/index.html`/`.ru.html`/`.en.html` — endi UCHALASI HAM `/api/content?lang=...` dan JONLI o'qiydi (ilgari faqat UZ jonli edi, RU/EN umuman fetch qilmasdi). DEFAULTS faqat tarmoq/server ishlamay qolgan holat uchun zaxira sifatida qoldi.
    - `static/admin.html` — "Loyihalar" (cases) generic-editor tabi olib tashlandi (Portfolio CRUD bilan bir xil edi — TZ audit'ida topilgan ikkinchi manba endi umuman yo'q). Paketlar/Jamoa/Fikrlar/FAQ (qisqa) bo'limlariga UZ/RU/EN til-tab qo'shildi (Xizmatlar/Portfolio CRUD bilan bir xil UX). **Yon-ta'sirda topilgan xato tuzatildi**: "Fikrlar" (testimonials) bo'limi kodda mavjud edi (render funksiyasi, shablon), lekin sidebar'da havolasi YO'Q edi — umuman ochib bo'lmasdi; endi "Fikrlar (zaxira)" nomi bilan qo'shildi. Rasm/founder-holati kabi tilga bog'liq bo'lmagan maydonlar (`photo`, `role_type`) bitta tilda o'zgartirilsa, qolgan 2 tilga avtomatik ko'chiriladi (aks holda admin bitta rasmni 3 marta yuklashi kerak bo'lardi).
    - Test qilindi: eski (flat) va yangi (til-bo'yicha) baza holatlarida server to'liq ishga tushirildi (xatosiz), `GET/PUT/POST /api/admin/content` va `GET /api/content?lang=` barcha 3 tilda httpx orqali tekshirildi (bitta tilni o'zgartirish faqat o'sha tilga ta'sir qilishi tasdiqlandi), `pytest tests/` (23/23), barcha JS fayllar `node --check`. Haqiqiy lokal `promtchi.db`ga migratsiya qo'llanildi (nusxa olingandan keyin, natija tekshirilgach nusxa o'chirildi).
    - **Kuzatuv**: Chrome kengaytmasi bu sessiyada ham ulanmagan — RU/EN homepage'ning yangi jonli-fetch xatti-harakati faqat HTTP/JSON darajasida tekshirildi, brauzerda vizual render (masalan sekin tarmoqda DEFAULTS→jonli ma'lumotga o'tish paytidagi "flash") ko'rilmadi.

**Qolgan — QOLGAN ISHLARNING BARCHASI BIZNES MA'LUMOTIGA BOG'LIQ, texnik jihatdan bajarib bo'lmaydi (2026-09-16 kuni biznes bilan tasdiqlangan qarorlar):**
- **Statistika** (32/24/98%/3yr) — hozircha o'zgartirilmaydi, joriy holatda qoladi (biznes qarori).
- **LocalBusiness schema** — ofis olingach, aniq manzil/ish vaqti bilan qo'shiladi (biznes qarori).
- **Jamoa a'zolari** (`/biz-haqimizda/`dagi real ism/foto) — jamoa hozircha kichik bo'lgani uchun shart emas, "Bizning ekspertiza" bloki bilan qoladi (biznes qarori).
