...
**Navbatda (audit asosida, ustuvorlik tartibida) — QOLGAN ISHLARNING BARCHASI BIZNES MA'LUMOTIGA BOG'LIQ, texnik jihatdan bajarib bo'lmaydi:**
...
**Audit orqali topilgan, hali ochiq qolgan boshqa masalalar (texnik, lekin qo'shimcha qaror talab qiladi):**
...
- **FAQ ikkita parallel manba**: `/uz/faq/` sahifasi `app/content/faq.py`dagi qattiq yozilgan 24 ta savol-javobni ko'rsatadi (admin-tahrirlanmaydi); Bosh sahifadagi qisqa FAQ bo'limi esa `Content.data.faq` (DB, admin "FAQ" tab orqali tahrirlanadi, 3 ta seed) dan chiqadi. TZ 19-band "FAQ CRUD" talabini asosiy `/faq/` sahifasi qismida hali to'liq qanoatlantirmaydi. Birlashtirish (`/faq/`ni ham DB'dan o'qitish + 24 ta real savolni DB'ga ko'chirish) mumkin, lekin TZ.md 3-band FAQ javoblarini hali "business sign-off kutilmoqda" deb belgilaganidan — qaysi javoblar YAKUNIY ekani aniqlanmaguncha bu migratsiya keyinga qoldirildi (aks holda ikki marta ishlash xavfi bor).
